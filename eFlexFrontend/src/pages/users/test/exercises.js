...
import {
  clearStoredEndTime,
  clearTestAbandonedFlag,
  getStoredUserAnswers,
  storeUserAnswers,
  clearStoredUserAnswers,
  getStoredTimeData,
  storeTimeData,
  clearStoredTimeData,
...
const Exercises = () => {
  ...
  const {
    ...
    testId ? testId : courseId,
    testId ? "Test" : "LevelAssessmentTest"
  );
  const {
    executeSubmit,
  ...
  const [isRecommendationPopupOpen, setIsRecommendationPopupOpen] =
    useState(false);
  const [isCancelConfirmPopupOpen, setIsCancelConfirmPopupOpen] =
    useState(false);
  ...
  } = useTestLifecycle(exercises, exercises.testExerciseId);
  ...
  const handleConfirmSubmit = useCallback(async () => {
    ...
    if (!exercises || !exercises.testExerciseId) {
      ...
      return navigate(
        lessonId
          ? `/course/${courseId}/lesson/${lessonId}/test`
          : `/course/${courseId}`
      );
    }

    const answersToSubmit = Object.entries(userAnswers).map(
      ([questionId, answer]) => ({
    ...
    if (result && result.statusCode === 200 && result.data) {
      if (result.data.recommendation) {
        ...
        toast.success(
          result.data.message || "Nộp bài thành công! Không có gợi ý bài học."
        );
      ...
      toast.error(
        `Nộp bài thất bại: ${
          submitError || (result && result.message) || "Vui lòng thử lại."
        }`
      );
    }
  }, [
  ...
  const confirmRecommendationNavigation = useCallback(() => {
    ...
    if (
      ...
      recommendationDetails.recommendation.lessonId
    ) {
      navigate(
        `/course/${courseId}/lesson/${recommendationDetails.recommendation.lessonId}`
      );
    }
  ...
  if (exerciseLoading || userDataLoading)
    return (
      <div className="exercises">
        <Loader text="Đang tải dữ liệu bài tập..." />
      </div>
    );
  if (exerciseError || userDataError)
    return (
      <div className="exercises">
        <Error
          Title={`Lỗi tải bài tập: ${
            exerciseError.message || String(exerciseError)
          }`}
        />
      </div>
    );
  if (!exercises) {
    return (
      <div className="exercises">
        <Error Title="Không tìm thấy dữ liệu bài tập hoặc bài tập không có câu hỏi." />
      </div>
    );
  }
  ...
  const recommendationPopupMessage = recommendationDetails
    ? `${recommendationDetails.recommendation.message}. Bấm xác nhận để chuyển đến bài học ngay.`
    : "";
