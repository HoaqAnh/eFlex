...
export const useSubmitTest = () => {
  ...
  const executeSubmit = async (userId, answers, type) => {
    if (!userId || !answers) {
      const errMsg =
        "Thông tin userId hoặc answers không được để trống khi nộp bài.";
    ...
    try {
      ...
      setError(null);
      setLoading(false);
      return await submitTestService(userId, answers, type);
    } catch (err) {
      setError(err.message || "Có lỗi trong quá trình gửi dữ liệu.");
      setLoading(false);
      return {
        statusCode: err.statusCode || 500,
        message: err.message || "Có lỗi trong quá trình gửi dữ liệu.",
        data: null,
      };
    }
  };
